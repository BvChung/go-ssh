...
func NewModel() Model {
	model := Model{width: maxWidth}
	model.lg = lipgloss.DefaultRenderer()
	model.styles = NewStyles(model.lg)
	model.state = FormActive
...
func getUser() tea.Cmd {
	return func() tea.Msg {
		time.Sleep(5 * time.Second)
		return userData{username: "Brandon"}
	}
}
...
func (m Model) View() string {
	...
	default:
		if m.username != "" {
			return lipgloss.NewStyle().Foreground(lipgloss.Color("#AA99FF")).Render(fmt.Sprintf("Username is %s\n", m.username))
		} else {
			return ""
		}
	}
